...
const itemCtrl = (function(){
    //item contructor - contructor de item
    ...
    //public method - metodos publicos
    return{
        logData: function(){
            return data;
        },
        addMoney: function(description, amount){
            //create random id - crear id
            let ID = itemCtrl.createID();
            //create new item
            newMoney = new Item(ID, description, amount);
            //push it into the array - agregar a arreglo
            data.items.push(newMoney);

            return newMoney;
        },
        createID: function(){
            //create random id number between 0 and 10000 - crear de manera aleatoria valores
            const idNum = Math.floor(Math.random()*10000);
            return idNum;
        },
        getIdNumber: function(item){
            //get the item id - identificar los item por id
        ...
        deleteAmountArr: function(id){
            ...
            const ids = data.items.map(function(item){
                //return item with id - retornar item con id
                return item.id
            });
            //get index
            const index = ids.indexOf(id)
            //remove item - remover por item
